...
public class NotationController {
    ...
    public String create( @Valid Notation notation, BindingResult result, Model model ) {

        if ( result.hasErrors() ) {
            model.addAttribute( "etudiants", userRepository.findByPromo( notation.getEtudiant().getPromo() ) );
            model.addAttribute( "matieres", matiereRepository.findByPromo( notation.getMatiere().getPromo() ) );
            return "note/new";
        }
